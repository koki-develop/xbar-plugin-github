...
const config = {
  ...
  github_host: process.env["GITHUB_HOST"] ?? "github.com",
...
const api_endpoint = config.github_host === "github.com" ? "api.github.com" : `${config.github_host}/api/v3`;
const graphql_api_endpoint = config.github_host === "github.com" ? "api.github.com" : `${config.github_host}/api`;
...
`;
  ...
  data = await fetch(`https://${graphql_api_endpoint}/graphql`, {
    method: "POST",
    headers: {
...
const fetchNotifications = async (max) => {
  const notifications = await fetch(
    `https://${api_endpoint}/notifications?per_page=${max + 1}`,
    {
      headers: {
        Authorization: `Bearer ${config.token}`,
      },
...
const readNotification = async (id) => {
  await fetch(`https://${api_endpoint}/notifications/threads/${id}`, {
    method: "PATCH",
    headers: {
      Authorization: `Bearer ${config.token}`,
    },
...
const readAllNotifications = async () => {
  const resp = await fetch(`https://${api_endpoint}/notifications`, {
    method: "PUT",
    headers: {
      Authorization: `Bearer ${config.token}`,
    },
...
(async () => {
  ...
  if (config.showReviewRequested) {
    const promise = fetchPullRequestsReviewRequested().then((pullRequests) => {
      ...
      reviewRequestedLines.push(
        `:eyes: Review Requested (${pullRequests.length}) | color=red href=https://${api_endpoint}/search?q=${encodeURIComponent(buildQueryPullRequestsReviewRequested())}`
      );
  ...
  if (config.showMyPullRequests) {
    const promise = fetchPullRequestsMine().then((pullRequests) => {
      ...
      const byRepo = groupResourcesByRepo(pullRequests);
      mineLines.push(
        `:seedling: My Pull Requests (${pullRequests.length}) | color=green href=https://${api_endpoint}/search?q=${encodeURIComponent(buildQueryPullRequestsMine())}`
      );
      for (const [repo, pullRequests] of Object.entries(byRepo)) {
        mineLines.push(
  ...
  if (config.showNotifications) {
    ...
    const promise = fetchNotifications(max).then(([notifications, hasMore]) => {
      ...
      notificationsLines.push(
        `:bell: Notifications (${count}) | color=yellow href=https://${api_endpoint}/notifications`
      );
      notificationsLines.push(
