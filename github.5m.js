...
// <xbar.var>boolean(SHOW_BOT_PULL_REQUESTS=false): Show Pull Requests created by bots.</xbar.var>
...
const config = {
  ...
  showBotPullRequests: process.env["SHOW_BOT_PULL_REQUESTS"] === "true",
};
...
const fetchPullRequestsReviewRequested = async () => {
  ...
  if (!config.showBotPullRequests) {
    filters.push(...botNames.map((botName) => `-author:app/${botName}`));
  }

  return await searchPullRequests(filters.join(" "));
};
